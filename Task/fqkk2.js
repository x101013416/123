...
let url = {
        url : "http://m."+fqkkurl.match(/m.(.*?).top/)[1]+".top/readb/finishTask",
...
function fqkk2(timeout = 0) {
  return new Promise((resolve) => {
let url = {
        url : "http://m."+fqkkurl.match(/m.(.*?).top/)[1]+".top/readb/jump?key="+fqkey,
        headers : JSON.parse(fqkkhd),
...
let url = {
        url : "http://m."+fqkkurl.match(/m.(.*?).top/)[1]+".top/readb/getTask",
        headers : JSON.parse(fqkkhd),
        body : '',}
      $.post(url, async (err, resp, data) => {
        try {
