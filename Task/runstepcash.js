...
if ($.isNode()) {
  ...
  if (
    process.env.RUNSTEPTOKEN &&
    process.env.RUNSTEPTOKEN.indexOf(COOKIES_SPLIT) > -1
  ) {
    runsteptokenVal = process.env.RUNSTEPTOKEN.split(COOKIES_SPLIT);
  } else {
    runsteptokenVal = process.env.RUNSTEPTOKEN.split();
  }
  if (
    process.env.RUNSTEPKEY &&
    process.env.RUNSTEPKEY.indexOf(COOKIES_SPLIT) > -1
  ) {
    runstepkeyVal = process.env.RUNSTEPKEY.split(COOKIES_SPLIT);
  } else {
    runstepkeyVal = process.env.RUNSTEPKEY.split();
  }
  if (
    process.env.TXTOKEN &&
    process.env.TXTOKEN.indexOf(COOKIES_SPLIT) > -1
  ) {
    txtokenVal = process.env.TXTOKEN.split(COOKIES_SPLIT);
  } else {
    txtokenVal = process.env.TXTOKEN.split();
  }
  if (
    process.env.TXKEY &&
    process.env.TXKEY.indexOf(COOKIES_SPLIT) > -1
  ) {
    txkeyVal = process.env.TXKEY.split(COOKIES_SPLIT);
  } else {
    txkeyVal = process.env.TXKEY.split();
  }
